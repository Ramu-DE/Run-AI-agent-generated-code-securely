#!/usr/bin/env bash
# One-command image build for the Module 2 callback-enabled reviewer.
# Replaces Step 2 of Module 2's durable-orchestration lab.
#
...
#
# On success, persists IMAGE_ARN to /etc/profile.d/mvm-image-arn.sh so
# every subsequent shell (and orchestrator/deploy.sh) picks it up.
set -euo pipefail
...
IMAGE_NAME="${IMAGE_NAME:-mvm-claude-reviewer-async}"
IMAGE_VERSION="${IMAGE_VERSION:-1.0}"

# Claude Code uses Bedrock via IAM role credentials — no API key needed.
# The execution role attached to the MicroVM has bedrock:InvokeModel* permissions.
