#!/usr/bin/env bash
# One-command image build for the Kiro callback-enabled reviewer.
#
# Prereqs (pre-populated by the workshop code editor bootstrap):
#   AWS_REGION, AWS_ACCOUNTID, ARTIFACTS_BUCKET, MODULE2_REVIEWER_BUILD_ROLE_ARN
# Plus KIRO_API_KEY_SECRET_ARN — the KiroApiKeySecretArn value from the workshop
# Event Outputs. The secret must already hold your Kiro API key (see the Kiro
# tab of Step 1).
#
# On success, persists IMAGE_ARN to /etc/profile.d/mvm-image-arn.sh so every
# subsequent shell (and orchestrator/deploy.sh) picks it up.
set -euo pipefail
...
: "${KIRO_API_KEY_SECRET_ARN:?KIRO_API_KEY_SECRET_ARN must be set — copy KiroApiKeySecretArn from the workshop Event Outputs}"

ARTIFACTS_BUCKET="${ARTIFACTS_BUCKET:-lambda-mvm-workshop-artifacts-${AWS_ACCOUNTID}}"
IMAGE_NAME="${IMAGE_NAME:-mvm-kiro-reviewer-async}"
...
echo "==> zipping source"
rm -f reviewer.zip
zip -qr reviewer.zip app.py Dockerfile
...
RESP=$(aws lambda-microvms create-microvm-image \
  ...
  --environment-variables "KIRO_API_KEY_SECRET_ARN=${KIRO_API_KEY_SECRET_ARN}" \
  --region "${AWS_REGION}" \
...
EOF
sudo chmod 644 /etc/profile.d/mvm-image-arn.sh
export IMAGE_ARN

echo ""
echo "Image built: ${IMAGE_ARN}"
echo "Persisted to /etc/profile.d/mvm-image-arn.sh for future shells."
